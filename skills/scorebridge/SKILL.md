...
5. Create/import the initial score, then apply targeted editor changes through MCP. Batch known operations where supported. Check actual instrument identity and playback assignment, not just staff labels. When a part has the wrong standard instrument, call `setPartInstrument` with a MuseScore instrument ID; changing a MIDI program alone does not change MuseScore 4's audio resource. Internal MusicXML is permitted for efficient initial import, but is not a user deliverable. Missing MCP actions require an adapter extension or an explicitly reported software-operation fallback, not silently dropped music.
